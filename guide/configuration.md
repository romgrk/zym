...
| `agent.profiles`               | array   | codex + claude-acp | Named ACP agents in the launcher's agent dropdown, each `{ "name", "command" }` plus optional `models`/`permissionModes`/`efforts` option lists (`{ "value", "args"? }`) |
| `agent.acp.command`            | array   | `["npx","-y","@agentclientprotocol/codex-acp"]` | Legacy single ACP argv; when set it surfaces as the first profile |
