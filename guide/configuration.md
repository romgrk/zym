...
| `agent.profiles`               | array   | gemini + claude-acp | Named ACP agents in the launcher's agent dropdown, each `{ "name", "command" }` plus optional `models`/`permissionModes`/`efforts` option lists (`{ "value", "args"? }`) |
