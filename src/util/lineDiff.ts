...
function myers(
  ...
  const n = a1 - a0;
  const m = b1 - b0;
  // The edit distance is at least |n − m|; past the bound the search can only fail, so skip the
  // whole D sweep (e.g. a view splice that replaces a large region with a few header rows).
  if (Math.abs(n - m) > maxD) return false;
  const max = n + m;
